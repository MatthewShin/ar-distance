...
sealed class MeasurementState {
    /** 앱 시작 직후 — 1타겟/2타겟, 일반/고성능 모드를 선택해야 하는 상태. */
    data object SelectingMode : MeasurementState()

    /** 박스1(1타겟 모드에서는 유일한 박스)을 아직 지정하지 않은 상태. */
    data class SelectingBox1(val mode: MeasurementMode, val accuracyMode: AccuracyMode) : MeasurementState()

    /** (2타겟 모드 전용) 박스1은 확정됐고, 박스2를 지정해야 하는 상태. */
    data class SelectingBox2(
    ...
    data class Tracking(
        ...
        /** 고성능 모드(다중 시점 삼각측량) 스냅샷 정밀측정 진행 상태. */
        val precisionCapture: PrecisionCaptureState = PrecisionCaptureState.Idle,
    ) : MeasurementState()
}

/** [MeasurementState.Tracking.precisionCapture]의 세부 상태 — 고성능 모드 전용. */
